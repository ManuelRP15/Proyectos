...
try {
    // Conectar a la base de datos usando la función conectarPDO
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $perfil_id = $_POST['perfil_id']; // Obtener el perfil seleccionado
        ...
        if ($usuarioExistente) {
            ...
            if ($stmt->execute()) {
                // Crear el directorio emails si no existe
                if (!file_exists('emails')) {
                    mkdir('emails', 0777, true);
                }

                // Enviar correo de verificación
                $verificationLink = "http://localhost:3000/acceso/validar.php?token=$token";
                $subject = "Verificación de cuenta";
                $message = "Haz clic en el siguiente enlace para verificar tu correo electrónico: $verificationLink";
                $headers = "From: no-reply@tu-dominio.com";

                // Guardar el correo en un archivo en la carpeta del proyecto
