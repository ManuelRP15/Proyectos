...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $visada = ($_SESSION['usuario']['perfil_id'] == 1) ? 1 : 0; // Si es administrador, visada = 1, si es ofertante, visada = 0
    $created_at = date('Y-m-d H:i:s');
    $updated_at = date('Y-m-d H:i:s');

    $query = "INSERT INTO ofertas (nombre, descripcion, fecha_actividad, aforo, categoria_id, usuario_id, visada, created_at, updated_at) VALUES (:nombre, :descripcion, :fecha_actividad, :aforo, :categoria_id, :usuario_id, :visada, :created_at, :updated_at)";
    $stmt = $conexion->prepare($query);
